...
def preamble(session, prices: dict, question: str) -> str:
    """首輪 user message:快照 + 行為背景 + 五角色結構要求 + 問題(原版 preamble)。"""
    return (
...
def convene(session, prices, history, question, *, api_key=None, base_url=None,
    ...
    user_content = (preamble(session, prices, question) if not history
                    else question)
